...
const users = await userModel
  ...
  .populate("organisation") // ← Type changes automatically!
  .toJSON();
...
> **💡 Info:** The populate API automatically transforms return types. Call `.populate("organisation")` and TypeScript knows that field is no longer an ID!
...
Stop casting to `any` and losing type safety. Monkko preserves types through your entire query chain:

```typescript
// Without populate
const user = await userModel.findById(userId);
// user.organisation → ObjectId
